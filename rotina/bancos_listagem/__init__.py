...
def bancos_listagem(log_email_exito, log_email_falha, bkp_data, log_dir, tipo_backup, dir_bkp_destino, dir_bkp_frio, user_banco, servidor_banco, porta_banco=5431):
    lista_bancos = ['psql', '-U', user_banco, '-h', f'{servidor_banco}', '-p', f'{porta_banco}', '-d', 'postgres', '-t', '-c', 'SELECT datname from pg_database WHERE datname not in (\'template0\', \'template1\', \'postgres\') order by 1']
    ...
    Thread(target=gerar_dump, args=(lista_bancos, log_email_exito, log_email_falha, tipo_backup, dir_bkp_destino, bkp_data, log_dir, dir_bkp_frio, user_banco, servidor_banco, porta_banco)).start()
